...
def plot_comparison(metric_name: str, public_labels: np.ndarray, private_labels: np.ndarray, plots_dir: str):
    ...
    fig.suptitle(f'Comparison of Rounded {metric_name} Distributions: Public vs. Private (DEAP)', fontsize=20, y=0.98)
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    
    plot_filename = os.path.join(plots_dir, f'DEAP_comparison_rounded_{metric_name.lower()}.png')
...
if __name__ == '__main__':
    ...
    arousal_pub_raw = load_labels('PUBLIC', 'arousal_pubblica') # Keep original metric names as they refer to filenames
    arousal_priv_raw = load_labels('PRIVATE', 'arousal_privata') # Keep original metric names as they refer to filenames
    ...
    valence_pub_raw = load_labels('PUBLIC', 'valence_pubblica') # Keep original metric names as they refer to filenames
    valence_priv_raw = load_labels('PRIVATE', 'valence_privata') # Keep original metric names as they refer to filenames
