...
class Attention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.):
        ...
        # Variabile per salvare la mappa di attenzione
        self.last_att = None
    ...
    def forward(self, x):
        ...
        # Salviamo la mappa di attenzione (detach e spostata su CPU per la visualizzazione)
        self.last_att = attn.detach().cpu()
...
class Deformer(nn.Module):
    ...
    def __init__(self, *, num_chan, num_time, temporal_kernel, num_kernel=64,
        ...
        # Calculate the correct dim after cnn_encoder dynamically
        padding_time = int(0.5 * (temporal_kernel - 1))
        cnn_out_time_dim = num_time  - (temporal_kernel - 1) # Time dim after conv1d (assuming padding=0 initially for calculation)
        cnn_out_time_dim = (cnn_out_time_dim + 2*padding_time - (1)) # Time dim after conv1d with padding
        cnn_out_time_dim = (cnn_out_time_dim - 2) // 2 + 1 # Time dim after maxpool2d
        dim = cnn_out_time_dim # Use the calculated time dimension

        #dim = int(0.5*num_time)  # embedding size after the first cnn encoder RIMOSSO DOPO SUGGERIMENTI AI
